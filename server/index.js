...
const { loadFromGitHub, getDB, saveDB } = require('./db');
...
function auth(req, res, next) {
  ...
  try { req.admin = jwt.verify(h.slice(7), db.jwt_secret); next(); } catch { res.status(401).json({ error: 'Invalid token' }); }
}
...
app.post('/api/verify', (req, res) => {
  ...
  if (!k.used) { k.used = true; k.hwid = hwid; saveDB(); log('key_activate', `${key} -> ${product.name} (${hwid})`, req.ip); }
...
app.post('/api/login', (req, res) => {
  ...
  res.json({ token: jwt.sign({ id: a.id, username: a.username }, db.jwt_secret, { expiresIn: '7d' }) });
});
...
app.get('/api/products', auth, (req, res) => { const db = getDB(); res.json(db.products.sort((a, b) => b.id - a.id)); });
app.post('/api/products', auth, (req, res) => {
  ...
  db.products.push(p); saveDB(); log('product_add', p.name, req.ip); res.json(p);
});
...
app.delete('/api/products/:id', auth, (req, res) => {
  ...
  saveDB(); log('product_delete', `#${pid}`, req.ip); res.json({ ok: true });
});
...
app.post('/api/products/:id/files', auth, upload.single('file'), (req, res) => {
  ...
  try {
    ...
    const extractDir = path.join(__dirname, '..', 'data', 'products', String(p.id));
    ...
    saveDB(); log('files_upload', `${p.name}: ${req.file.originalname}`, req.ip); res.json({ ok: true });
...
app.post('/api/keys', auth, (req, res) => {
  ...
  saveDB(); log('keys_gen', `${n} keys for #${product_id}`, req.ip); res.json(newKeys);
});
app.put('/api/keys/:id', auth, (req, res) => {
  const db = getDB();
  const k = db.keys.find(x => x.id === +req.params.id);
  if (!k) return res.status(404).json({ error: 'Not found' });
  Object.assign(k, req.body); saveDB(); res.json(k);
});
app.delete('/api/keys/:id', auth, (req, res) => {
  const db = getDB();
  db.keys = db.keys.filter(k => k.id !== +req.params.id); saveDB(); res.json({ ok: true });
});
app.post('/api/keys/:id/reset', auth, (req, res) => {
  const db = getDB();
  const k = db.keys.find(x => x.id === +req.params.id);
  if (!k) return res.status(404).json({ error: 'Not found' });
  k.used = false; k.hwid = ''; saveDB(); log('key_reset', k.value, req.ip); res.json(k);
});

// ========== CLIENTS ==========

app.get('/api/clients', auth, (req, res) => { const db = getDB(); res.json(db.clients.sort((a, b) => new Date(b.last_seen) - new Date(a.last_seen))); });
app.delete('/api/clients/:id', auth, (req, res) => {
  const db = getDB();
  db.clients = db.clients.filter(c => c.id !== +req.params.id); saveDB(); res.json({ ok: true });
});
...
app.get('/api/settings', auth, (req, res) => { const db = getDB(); res.json(db.settings); });
app.put('/api/settings', auth, (req, res) => {
  const db = getDB();
  const { key, value } = req.body;
  if (key) { db.settings[key] = value; saveDB(); }
  res.json({ ok: true });
});
app.post('/api/password', auth, (req, res) => {
  ...
  a.password = bcrypt.hashSync(password, 10); saveDB(); log('password_change', a.username, req.ip); res.json({ ok: true });
});
...
setInterval(() => {
  ...
  const now = Date.now();
  let changed = false;
  for (const [k, v] of Object.entries(db._tokens || {})) { if (now - v.ts > 600000) { delete db._tokens[k]; changed = true; } }
  if (changed) saveDB();
}, 60000);
...
(async () => {
  ...
  const db = getDB();
  if (!db.jwt_secret) { db.jwt_secret = crypto.randomBytes(32).toString('hex'); saveDB(); }
  if (!db.admins.length) {
  ...
  console.log(`DB: ${db.products.length} products, ${db.keys.length} keys, ${db.clients.length} clients`);
